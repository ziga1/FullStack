...
resource "azurerm_public_ip" "terra-demo-vm1" {
...
resource "azurerm_network_interface" "terra-demo-vm1" {
  name                = "terra-demo-nic1"
  location            = azurerm_resource_group.terra-demo.location
  resource_group_name = azurerm_resource_group.terra-demo.name
  ...
  ip_configuration {
    ...
    public_ip_address_id          = azurerm_public_ip.terra-demo-vm1.id
  }
}

resource "azurerm_linux_virtual_machine" "terra-demo-vm1" {
  ...
  network_interface_ids = [
    azurerm_network_interface.terra-demo-vm1.id,
  ]
